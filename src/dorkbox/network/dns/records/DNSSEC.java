...
class DNSSEC {
     ...
     */
    public static
    byte[] digestMessage(SIGRecord sig, DnsMessage msg, byte[] previous) {
     ...
     */
    public static
    class UnsupportedAlgorithmException extends DNSSECException {
     ...
     */
    public static
    class MalformedKeyException extends DNSSECException {
        MalformedKeyException(KEYBase rec) {
            super("Invalid key data: " + asString(rec));
        }

        private static
        String asString(final KEYBase rec) {
     ...
     */
    public static
    class KeyMismatchException extends DNSSECException {
        private KEYBase key;
        private SIGBase sig;
     ...
     */
    public static
    class SignatureExpiredException extends DNSSECException {
     ...
     */
    public static
    class SignatureNotYetValidException extends DNSSECException {
        private Date when, now;
     ...
     */
    public static
    class SignatureVerificationException extends DNSSECException {
     ...
     */
    public static
    class IncompatibleKeyException extends IllegalArgumentException {
     ...
     */
    public static
    class NoSignatureException extends DNSSECException {
    ...
    private static
    void writeBigInteger(DnsOutput out, BigInteger val) {
        byte[] b = trimByteArray(val.toByteArray());
        out.writeByteArray(b);
    }

    private static
    void writePaddedBigInteger(DnsOutput out, BigInteger val, int len) {
        byte[] b = trimByteArray(val.toByteArray());
    ...
    private static
    void writePaddedBigIntegerLittleEndian(DnsOutput out, BigInteger val, int len) {
        byte[] b = trimByteArray(val.toByteArray());
     ...
     */
    static
    PublicKey toPublicKey(KEYBase r) throws DNSSECException {
    ...
    private static
    PublicKey toRSAPublicKey(KEYBase r) throws IOException, GeneralSecurityException {
    ...
    private static
    BigInteger readBigInteger(DnsInput in, int len) throws IOException {
        byte[] b = in.readByteArray(len);
        return new BigInteger(1, b);
    }

    private static
    BigInteger readBigInteger(DnsInput in) {
        byte[] b = in.readByteArray();
        return new BigInteger(1, b);
    }

    private static
    PublicKey toDSAPublicKey(KEYBase r) throws IOException, GeneralSecurityException, MalformedKeyException {
        DnsInput in = new DnsInput(r.getKey());
    ...
    private static
    PublicKey toECGOSTPublicKey(KEYBase r, ECKeyInfo keyinfo) throws IOException, GeneralSecurityException, MalformedKeyException {
        DnsInput in = new DnsInput(r.getKey());
    ...
    private static
    BigInteger readBigIntegerLittleEndian(DnsInput in, int len) throws IOException {
        byte[] b = in.readByteArray(len);
        reverseByteArray(b);
        return new BigInteger(1, b);
    }
    ...
    private static
    PublicKey toECDSAPublicKey(KEYBase r, ECKeyInfo keyinfo) throws IOException, GeneralSecurityException, MalformedKeyException {
        DnsInput in = new DnsInput(r.getKey());
    ...
    private static
    byte[] fromRSAPublicKey(RSAPublicKey key) {
        DnsOutput out = new DnsOutput();
    ...
    private static
    byte[] fromDSAPublicKey(DSAPublicKey key) {
        DnsOutput out = new DnsOutput();
        BigInteger q = key.getParams()
                          .getQ();
        BigInteger p = key.getParams()
                          .getP();
        BigInteger g = key.getParams()
                          .getG();
    ...
    private static
    byte[] fromECGOSTPublicKey(ECPublicKey key, ECKeyInfo keyinfo) {
        DnsOutput out = new DnsOutput();

        BigInteger x = key.getW()
                          .getAffineX();
        BigInteger y = key.getW()
                          .getAffineY();
    ...
    private static
    byte[] fromECDSAPublicKey(ECPublicKey key, ECKeyInfo keyinfo) {
        DnsOutput out = new DnsOutput();
     ...
     */
    static
    byte[] fromPublicKey(PublicKey key, int alg) throws DNSSECException {
        switch (alg) {
            case Algorithm.RSAMD5:
            case Algorithm.RSASHA1:
            case Algorithm.RSA_NSEC3_SHA1:
     ...
     */
    public static
    void verify(RRset rrset, RRSIGRecord rrsig, DNSKEYRecord key) throws DNSSECException {
     ...
     */
    public static
    byte[] digestRRset(RRSIGRecord rrsig, RRset rrset) {
        DnsOutput out = new DnsOutput();
        digestSIG(out, rrsig);

        int size = rrset.size();
        DnsRecord[] records = new DnsRecord[size];
        ...
        while (it.hasNext()) {
            records[--size] = (DnsRecord) it.next();
        }
        Arrays.sort(records);
    ...
    private static
    void verify(PublicKey key, int alg, byte[] data, byte[] signature) throws DNSSECException {
        if (key instanceof DSAPublicKey) {
            try {
     ...
     */
    public static
    String algString(int alg) throws UnsupportedAlgorithmException {
        switch (alg) {
            case Algorithm.RSAMD5:
                return "MD5withRSA";
            case Algorithm.DSA:
    ...
    private static
    byte[] DSASignaturefromDNS(byte[] dns) throws DNSSECException, IOException {
    ...
    private static
    byte[] ECGOSTSignaturefromDNS(byte[] signature, ECKeyInfo keyinfo) throws DNSSECException, IOException {
    ...
    private static
    byte[] ECDSASignaturefromDNS(byte[] signature, ECKeyInfo keyinfo) throws DNSSECException, IOException {
        if (signature.length != keyinfo.length * 2) {
            throw new SignatureVerificationException();
        }
    ...
    private static
    boolean matches(SIGBase sig, KEYBase key) {
     ...
     */
    public static
    RRSIGRecord sign(RRset rrset, DNSKEYRecord key, PrivateKey privkey, Date inception, Date expiration) throws DNSSECException {
     ...
     */
    public static
    RRSIGRecord sign(RRset rrset, DNSKEYRecord key, PrivateKey privkey, Date inception, Date expiration, String provider)
            throws DNSSECException {
        int alg = key.getAlgorithm();
        checkAlgorithm(privkey, alg);

        RRSIGRecord rrsig = new RRSIGRecord(rrset.getName(),
                                            rrset.getDClass(),
                                            rrset.getTTL(),
                                            rrset.getType(),
                                            alg,
    ...
    private static
    byte[] sign(PrivateKey privkey, PublicKey pubkey, int alg, byte[] data, String provider) throws DNSSECException {
        byte[] signature;
        try {
            Signature s;
            if (provider != null) {
    ...
    private static
    int BigIntegerLength(BigInteger i) {
        return (i.bitLength() + 7) / 8;
    }

    private static
    byte[] DSASignaturetoDNS(byte[] signature, int t) throws IOException {
        DnsInput in = new DnsInput(signature);
        DnsOutput out = new DnsOutput();
    ...
    private static
    byte[] ECDSASignaturetoDNS(byte[] signature, ECKeyInfo keyinfo) throws IOException {
        DnsInput in = new DnsInput(signature);
        DnsOutput out = new DnsOutput();
    ...
    static
    void checkAlgorithm(PrivateKey key, int alg) throws UnsupportedAlgorithmException {
        switch (alg) {
            case Algorithm.RSAMD5:
            case Algorithm.RSASHA1:
            case Algorithm.RSA_NSEC3_SHA1:
    ...
    static
    SIGRecord signMessage(DnsMessage dnsMessage, SIGRecord previous, KEYRecord key, PrivateKey privkey, Date inception, Date expiration)
            throws DNSSECException {
        int alg = key.getAlgorithm();
        checkAlgorithm(privkey, alg);

        SIGRecord sig = new SIGRecord(Name.root, DnsClass.ANY, 0, 0, alg, 0, expiration, inception, key.getFootprint(), key.getName(), null);
        DnsOutput out = new DnsOutput();
        digestSIG(out, sig);
        if (previous != null) {
    ...
    static
    void verifyMessage(DnsMessage dnsMessage, byte[] bytes, SIGRecord sig, SIGRecord previous, KEYRecord key) throws DNSSECException {
        ...
        Header header = (Header) dnsMessage.getHeader()
                                           .clone();
     ...
     */
    static
    byte[] generateDSDigest(DNSKEYRecord key, int digestid) {
