...
class ARecordTest : TestCase() {
    ...
    fun test_ctor_0arg() {
        ...
        assertEquals(InetAddress.getByName("0.0.0.0"), ar.address)
    }

    fun test_getObject() {
        val ar = ARecord()
        val r = ar.dnsRecord
        assertTrue(r is ARecord)
    }

    fun test_ctor_4arg() {
        val ar = ARecord(m_an, DnsClass.IN, m_ttl, m_addr)
        assertEquals(m_an, ar.name)
        assertEquals(DnsRecordType.A, ar.type)
        ...
        // a relative name
        try {
            ARecord(m_rn, DnsClass.IN, m_ttl, m_addr)
        ...
        // an IPv6 address
        try {
            ARecord(m_an, DnsClass.IN, m_ttl, InetAddress.getByName("2001:0db8:85a3:08d3:1319:8a2e:0370:7334"))
