...
class MFRecordTest : TestCase() {
    ...
    fun test_ctor_4arg() {
        ...
        val d = MFRecord(n, DnsClass.IN, 0xABCDEL, a)
        assertEquals(n, d.name)
        assertEquals(DnsRecordType.MF, d.type)
        assertEquals(DnsClass.IN, d.dclass)
        assertEquals(0xABCDEL, d.ttl)
        assertEquals(a, d.additionalName)
        assertEquals(a, d.mailAgent)
    }

    fun test_getObject() {
        val d = MFRecord()
        val r = d.`object`
        assertTrue(r is MFRecord)
    }
}
